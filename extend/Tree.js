layui.define(['jquery'], function(exports) {
	...
	tree.prototype.checked = function(e) {
		var checkedData = [];
		// 循环当前选中id
		this.checkedId(e).forEach(function(id){
			// 循环所有数据
			e.data.forEach(function(el, i){
				if(id == el.id) checkedData.push(el);
			});
		});
		return checkedData;
	};
	...
	tree.prototype.treeDown = function(e){
		if(!e.is_checkbox){
			...
			// 当前选中
			if(e.checked.data){
				e.checked.data.forEach(function(value){
					e.data.forEach(function(el){
						if(value == el.id) obj_down.val(el.name);
					});
				});
			}
			// 点击下拉
